...
function Artisan.setup()
    ...
    Artisan.add('migrate:fresh', {alias = 'mgf', description = 'Drop all tables and re-run all migrations'})
    Artisan.add('migrate:rollback', {alias = 'mgr', description = 'Rollback the last database migration'})
    Artisan.add('migrate:fresh --seed', {alias = 'mgfs', description = 'Drop all tables, re-run migrations, and seed'})
    Artisan.add('db:seed', {alias = 'ds', description = 'Seed the database with records'})

    Artisan.add('route:list --columns=method,uri,name,action', {alias = 'rl', description = 'List all registered routes', callback = function()
        output = hs.execute(
            table.concat({'/usr/local/bin/php', Artisan.path, Artisan.command, '--json'}, ' ')
        )
    ...
    Artisan.add('--version', {alias = 'v', description = 'Display this application version'})
    Artisan.add('test --parallel', {alias = 'tp', description = 'Run the application tests in parallel'})
    Artisan.add('cache:clear', {alias = 'cc', description = 'Flush the application cache'})
    Artisan.add('key:generate', {alias = 'kg', description = 'Set the application key'})
    Artisan.add('storage:link', {alias = 'sl', description = 'Create the symbolic links configured for the application'})
    Artisan.add('stub:publish', {alias = 'sp', description = 'Publish all stubs that are available for customization'})
    Artisan.add('vendor:publish', {alias = 'vp', description = 'Publish any publishable assets from vendor packages'})
    Artisan.add('view:clear', {alias = 'vc', description = 'Flush the application cache'})

    Artisan.add('make:cast', {alias = 'mca', args = 'name', description = 'Create a new custom Eloquent cast class'})
    Artisan.add('make:channel', {alias = 'mch', args = 'name', description = 'Create a new channel class'})
    Artisan.add('make:command', {alias = 'mcmd', args = 'name', description = 'Create a new Artisan command'})
    Artisan.add('make:component', {alias = 'mco', args = 'name', description = 'Create a new view component class'})
    Artisan.add('make:controller {$1}Controller', {alias = 'mc', args = 'name', description = 'Create a new controller class'})
    Artisan.add('make:event', {alias = 'mev', args = 'name', description = 'Create a new event class'})
    Artisan.add('make:exception', {alias = 'mexc', args = 'name', description = 'Create a new custom exception class'})
    ...
    Artisan.add('make:migration create_{$1}_table --create={$1}', {alias = 'mmgc', args = 'name', description = 'Create a new migration file'})
    Artisan.add('make:model -f', {alias = 'mmd', args = 'name', description = 'Create a new Eloquent model class'})
    Artisan.add('make:notification', {alias = 'mn', args = 'name', description = 'Create a new notification class'})
    Artisan.add('make:observer', {alias = 'mo', args = 'name', description = 'Create a new observer class'})
    Artisan.add('make:policy {$1}Policy', {alias = 'mpo', args = 'name', description = 'Create a new policy class'})
    Artisan.add('make:provider', {alias = 'mpr', args = 'name', description = 'Create a new service provider class'})
    Artisan.add('make:request {$1}Request', {alias = 'mrq', args = 'name', description = 'Create a new form request class'})
    Artisan.add('make:resource', {alias = 'mrs', args = 'name', description = 'Create a new resource'})
    Artisan.add('make:rule', {alias = 'mrl', args = 'name', description = 'Create a new validation rule'})
...
function Artisan.loadAlfredJson()
    ...
    hs.json.write({items = items}, '/Users/nathan/.fuelingzsh/custom/artisan.json', false, true)
end
    ...
    hs.task.new(
        ...
        function(exitCode, stdOut, stdErr)
            if stdOut then
                ...
                if notify then
                    hs.notify.new({title = 'artisan ' .. Artisan.command, informativeText = stdOut}):send()
                end
            end
