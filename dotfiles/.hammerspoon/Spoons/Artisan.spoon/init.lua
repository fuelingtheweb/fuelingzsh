...
function Artisan.setup()
    ...
    Artisan.add('db:seed',
                {alias = 'ds', description = 'Seed the database with records'})

    Artisan.add('route:list --columns=method,uri,name,action', {
        alias = 'rl',
        description = 'List all registered routes',
        callback = function()
            output = hs.execute(table.concat({
                '/usr/local/bin/php', Artisan.path, Artisan.command, '--json'
            }, ' '))
            file = io.open(
                       '/Users/nathan/.fuelingzsh/custom/artisan-output.log',
                       'w')
            file:write(output)
            file:close()
    ...
    Artisan.add('--version',
                {alias = 'v', description = 'Display this application version'})
    Artisan.add('test --parallel', {
        alias = 'tp',
        description = 'Run the application tests in parallel'
    })
    Artisan.add('cache:clear',
                {alias = 'cc', description = 'Flush the application cache'})
    Artisan.add('key:generate',
                {alias = 'kg', description = 'Set the application key'})
    ...
    Artisan.add('view:clear',
                {alias = 'vc', description = 'Flush the application cache'})
    ...
    Artisan.add('make:view', {
        ...
        callback = function(params)
            Artisan.openNewFiles(function()
                ...
                filePath = Artisan.mainPath .. '/resources/views/' .. name ..
                               '.blade.php'
                filePath = filePath:gsub('~', '/Users/nathan')
                output = hs.execute('mkdir -p "$(dirname "' .. filePath ..
                                        '")" && touch "' .. filePath .. '"')
            end)
        end
    })
...
function Artisan.loadAlfredJson()
    ...
    hs.fnutils.each(Artisan.commands, function(command)
        ...
        table.insert(items, {
            ...
            arg = command.name:gsub(' ', ':s:'):gsub('{%$1}', ':1:'):gsub('=',
                                                                          ':eq:')
        })
    end)

    hs.json.write({items = items},
                  '/Users/nathan/.fuelingzsh/custom/artisan.json', false, true)
end
...
function Artisan.setupArgs()
    ...
    each(hs.fnutils.split(Artisan.command, ' '),
         function(arg) table.insert(args, arg) end)

    return args
end

function Artisan.run(notify)
    hs.task.new('/usr/local/bin/php', function(exitCode, stdOut, stdErr)
        if stdOut then
        ...
        if stdErr then Artisan.error(stdErr) end
    end, function(task, stdOut, stdErr)
        if stdOut then Artisan.success(stdOut) end

        if stdErr then Artisan.error(stdErr) end

        return true
    end, Artisan.setupArgs()):start()
end

function Artisan.success(value) Ray.green(value:gsub("[\n\r]", '<br>')) end

function Artisan.error(value) Ray.red(value:gsub("[\n\r]", '<br>')) end

hs.urlevent.bind('artisan', function(eventName, params)
    Artisan.command = params.command:gsub(':s:', ' '):gsub(':1:', '{$1}'):gsub(
                          ':eq:', '=')
    print(Artisan.command)
    command = Artisan.commands[Artisan.command]

    if command.args then
        return triggerAlfredWorkflow('artisan-command',
                                     'com.fuelingtheweb.commands', command.name)
    end
...
hs.urlevent.bind('artisan-command', function(eventName, params)
    ...
    if command.callback then return command.callback({name = params.name}) end
    ...
    Artisan.openNewFiles(function()
        ...
        parts = hs.fnutils.map(hs.fnutils.split(params.name, '/'),
                               function(text)
...
function Artisan.openNewFiles(callback)
    ...
    hs.timer.doAfter(0.5, function() Artisan.watcher:stop() end)
end

function Artisan.convertTo(converter, text)
    return trim(hs.execute(
